...
- **현재 22명 전수** 행이 들어 있음(2026-09-07). 파일럿 15명 + 비파일럿 7명.
  비파일럿 news_count 는 어디에도 저장 안 되지만(brand_fit_pilot.csv 밖), 타임라인·트렌드는
  player_news.json 에 들어가 선수 대시보드에서 전 선수에게 표시된다.
- 동명이인·흔한 이름(이건희=삼성 회장, 정재민/이상헌 등)은 note 에 노이즈 경고 표시 — 건수는 참고만.
