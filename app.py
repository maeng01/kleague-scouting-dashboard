...
파일럿 14명의 `news_count` = 네이버 뉴스 검색(NAVER API HUB) 고정 질의의 전체 건수(`total`).
`src/brand_fit.py` 가 `log10(news_count)` 를 0–100 으로 매핑해 media 축에 반영한다.
| 선수 | 뉴스 건수 | 선수 | 뉴스 건수 |
|---|---|---|---|
| 주민규 | 38,212 | 가브리엘(빅토르) | 1,618 |
| 무고사 | 18,654 | 디오고 | 1,328 |
| 마르캉(말컹) | 9,350 | 김신진 | 908 |
| 이호재 | 8,744 | 페리어 | 821 |
| 모따(Bruno) | 5,662 | 프리드욘손 | 49 |
| 야고 | 4,463 | 오로보·흘레이할 | ~30 |
| 클리말라 | 2,645 | | |

Breno 는 네이버 표기가 불명확(`브레노` 208건 과대 / `브레노 안양` 2건 과소)해 news_count 공란 → `media_exposure` 버킷 유지.
프리드욘손(49)·김신진(908)처럼 낮은 값은 "국내 언론에 개인으로 거의 안 나온다"는 것 자체가 신호다.
