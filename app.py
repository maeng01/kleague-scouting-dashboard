...
st.sidebar.caption(
    "**SNS**: 파일럿(스트라이커 11명)만 수동 수집. 참여율은 공개 IG 있는 7명 **실측**, "
    "나머지는 미수집."
)
...
- **참여율** : 최근 게시물 (좋아요+댓글) ÷ 팔로워. 팔로워 티어 추정이 아니라 **직접 집계**해야 의미가 있습니다. 파일럿 7명은 실측값(Marcão 103만 · Bruno Mota 등).
...
| SNS·참여율 | Instagram 공개 게시물 직접 집계 | 파일럿 7명 참여율 실측 |
...
- **SNS/이미지**: 파일럿(스트라이커 11명)만 수동 수집. 참여율은 공개 IG 있는 **7명 실측**
...
**참여율 실측 (파일럿 7명)** — IG 게시물별 (좋아요+댓글)/팔로워, 최근 3–10건 평균:
| 선수 | 팔로워 | 참여율 | 메모 |
|---|---|---|---|
...
→ **소규모 계정 구간(9K–45K)에서는 팔로워가 많을수록 참여율이 낮아진다** — Yago·Bruno Mota는
1만 미만 팔로워에 참여율 15–20%, Klimala는 4.5만에 6.7%. 팬덤이 작고 밀도가 높을수록 반응률이 높다.
→ 반면 **Marcão(103만·adidas 계약)은 다른 티어** — 100만 계정에서 참여율 3.2%는 이례적으로 높다
(대부분 1M+ 계정은 1% 미만). 즉 파일럿에서 "작지만 밀도 높은 팬덤" 논리가 필요 없는 유일한 선수.
Marketability 산식은 이걸 자동으로 반영 — Marcão는 도달(reach) 축이 만점이라 점수가 참여율이
아니라 규모에서 나온다.
...
- SNS는 공개 API 제약으로 수작업 수집. 참여율은 파일럿 7명 실측, 나머지 미수집.
- 언론 노출은 아직 수동 버킷 (재현 가능한 뉴스 건수로 전환 파이프라인은 구축, 값은 미수집).
