...
st.sidebar.caption(
    "**SNS**: 파일럿(스트라이커 15명)만 수동 수집. 참여율은 공개 IG 있는 11명 **실측**, "
    "나머지는 미수집."
)
...
- **참여율** : 최근 게시물 (좋아요+댓글) ÷ 팔로워. 팔로워 티어 추정이 아니라 **직접 집계**해야 의미가 있습니다. 파일럿 11명은 실측값(Marcão 103만 ~ Kim Sinjin 4K). Lee Hojae는 개인 계정 없음.
...
4. **② 탭** — Marketability 막대에서 어느 축이 점수를 끌고/눌렀는지 봅니다. Yago는 참여율 20.7%가 도달(팔로워 9.1K)을 상쇄해 71.5점. 카테고리 표 1·2위는 '스포츠웨어'·'게임·e스포츠'(젊은 유망주·화려한 플레이 태그).
...
| SNS·참여율 | Instagram 공개 게시물 직접 집계 | 파일럿 15명 중 11명 참여율 실측(+2명 팔로워만, 2명 계정없음/미확인) |
...
# ===========================================================================
    ...
    st.caption("선정: 22명 풀 중 **공개 개인 IG 계정이 확인되는 스트라이커 15명** "
               "(1차는 과정지표 상위 중심, 이후 풀 전반으로 확대). 이들만 SNS/이미지 수동 수집 → Marketability·Agency 산출.")
    rows = []
...
- **SNS/이미지**: 파일럿(스트라이커 15명)만 수동 수집. 참여율은 공개 IG 있는 **11명 실측**
  (게시물별 좋아요/댓글). 2026-08 로그인 없이 5명(og:description) → 2026-09 Instagram 이
  비로그인 접근을 막아, 로그인 세션(Claude in Chrome)으로 Marcão·Diogo·Breno·Hólmbert·Kim Sinjin 등 추가.
  Hleihil·Vitor Gabriel 은 좋아요 비공개라 팔로워만, Lee Hojae·Ferrier 는 개인 계정 없음/미확인.
...
파일럿 15명이 대부분 '외국인 저니맨 스트라이커'라 `global_journey`·`hardworking_pro` 태그가 겹쳐,
...
**참여율 실측 (파일럿 11명)** — IG 게시물별 좋아요(±댓글)/팔로워, 최근 3–10건 평균:
| 선수 | 팔로워 | 참여율 | 메모 |
|---|---|---|---|
| Kim Sinjin | 4.0K | **~23%** | 파일럿 최고 — 4K 초소형·극단적 밀도 |
| Yago Cariello | 9.1K | 20.7% | 소규모·고밀도 |
| Bruno Mota | 8.7K | ~15% | 좋아요만(댓글 제한 계정) |
| Hólmbert Friðjónsson | 8.7K | ~12% | 아이슬란드 대표·가정 콘텐츠 |
...
| Breno Herculano | 20K | ~2.7% | Skechers Korea 착용 |
| Diogo | 78K | ~1.3% | 78K 치고 낮음 — 비활성 팔로워 추정 |

→ **소규모 계정(4K–45K)에서는 팔로워가 많을수록 참여율이 낮아진다** — Kim Sinjin(4K)·Yago·Bruno Mota는
참여율 15–23%, Klimala는 4.5만에 6.7%. 팬덤이 작고 밀도가 높을수록 반응률이 높다.
Kim Sinjin(4K·23%)은 곡선의 저용량 끝점 — 참여율은 최고지만 도달(팔로워)이 거의 0이라 Marketability 상방은 낮다.
...
파일럿 15명 전원의 `news_count` = 네이버 뉴스 검색(NAVER API HUB) 고정 질의의 전체 건수(`total`).
`src/brand_fit.py` 가 `log10(news_count)` 를 0–100 으로 매핑해 media 축에 반영한다.
| 선수 | 뉴스 건수 | 선수 | 뉴스 건수 |
|---|---|---|---|
| 주민규 | 38,216 | 가브리엘(빅토르) | 1,619 |
| 무고사 | 18,654 | 디오고 | 1,331 |
| 마르캉(말컹) | 9,353 | 김신진 | 908 |
| 이호재 | 8,744 | 엘쿠라노(Breno) | 488 |
| 모따(Bruno) | 5,662 | 페리어 | 821 |
| 야고 | 4,469 | 프리드욘손 | 49 |
| 클리말라 | 2,645 | 오로보·흘레이할 | ~30 |

외국인 선수는 국내 등록명이 관건: 마르캉→**말컹**, Diogo→**디오고**, Vitor Gabriel→**빅토르 가브리엘**,
Breno Herculano→**엘쿠라노**(성 Herculano). 프리드욘손(49)·김신진(908)처럼 낮은 값은
"국내 언론에 개인으로 거의 안 나온다"는 것 자체가 신호다.
...
"브랜드 캠페인·재계약 타이밍" 참고 신호로만 표시(③ Agency 카드가 상승세→전략, 하락세→리스크로 반영).
22명 전수: 상승세 클리말라·야고·오로보·모따 / 하락세 페리어·Vitor Gabriel·공민현 등 / 검색량 미미하면 "데이터 부족".
갱신: `python -m src.collect_naver` (키 필요).
...
- SNS는 공개 API 제약으로 수작업 수집. 파일럿 15명 중 참여율 11명 실측(+2명 팔로워만·2명 계정없음/미확인).
