...
st.sidebar.caption(
    "**SNS**: 파일럿(스트라이커 11명)만 수동 수집. 참여율은 공개 IG 있는 6명 **실측**, "
    "나머지는 미수집."
)
...
- **참여율** : 최근 게시물 (좋아요+댓글) ÷ 팔로워. 팔로워 티어 추정이 아니라 **직접 집계**해야 의미가 있습니다. 파일럿 6명은 실측값(Marcão 포함, 103만 팔로워).
...
| SNS·참여율 | Instagram 공개 게시물 직접 집계 | 파일럿 6명 참여율 실측 |
...
- **SNS/이미지**: 파일럿(스트라이커 11명)만 수동 수집. 참여율은 공개 IG 있는 **6명 실측**
...
**참여율 실측 (파일럿 6명)** — IG 게시물별 (좋아요+댓글)/팔로워, 최근 3–10건 평균:
| 선수 | 팔로워 | 참여율 | 메모 |
|---|---|---|---|
...
→ **소규모 계정 구간(9K–45K)에서는 팔로워가 많을수록 참여율이 낮아진다** — Yago는 Klimala의
1/5 팔로워인데 참여율 3배. 팬덤이 작고 밀도가 높을수록 반응률이 높은 것.
...
- SNS는 공개 API 제약으로 수작업 수집. 참여율은 파일럿 6명 실측, 나머지 미수집.
