...
st.sidebar.caption(
    "**SNS**: 파일럿(스트라이커 11명)만 수동 수집(2026-08). 참여율은 공개 IG 있는 5명 **실측**, "
    "나머지는 미수집."
)
...
- **참여율** : 최근 게시물 (좋아요+댓글) ÷ 팔로워. 팔로워 티어 추정이 아니라 **직접 집계**해야 의미가 있습니다. 파일럿 5명은 실측값.
...
| SNS·참여율 | Instagram 공개 게시물 직접 집계 | 파일럿 5명 참여율 실측 |
...
- **SNS/이미지**: 파일럿(스트라이커 11명)만 수동 수집(2026-08). 참여율은 공개 IG 있는 5명에 대해
  **실측**(게시물별 좋아요/댓글, 최근 5–10건). 나머지는 미확인.
  2026-09 기준 Instagram 이 비로그인 프로필 접근을 차단해, 추가 실측은 로그인 세션이 있어야 가능.
...
- SNS는 공개 API 제약으로 수작업 수집. 참여율은 파일럿 5명 실측, 나머지 미수집.
